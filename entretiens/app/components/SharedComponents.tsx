...
// Composant Sidebar
export const Sidebar = ({ onLogout, open, palette, role, links }: { onLogout: () => void; open: boolean; palette: any; role?: number; links?: {label: string; icon: string; href: string; adminOnly?: boolean;}[] }) => (
	<aside
		style={{
			width: open ? 220 : 0,
	...
	>
		...
		<nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4, padding: "0 10px" }}>
			{(links || sidebarLinks)
				.filter(link => {
					// cacher les liens adminOnly si non-admin
					if (link.adminOnly && role !== 1) return false;
					// cacher "Utilisateurs" pour role basic (2)
					if (role === 2 && link.label === "Utilisateurs") return false;
					return true;
				})
				.map(link => (
				<a
